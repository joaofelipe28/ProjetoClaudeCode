...
const ALLOC_COLORS = { 'Renda Fixa Longo': '#2563eb', 'Renda Fixa Curto': '#16a34a', 'Renda Variável': '#ea580c' }
...
export function Investimentos() {
  ...
  const taxaMedia = investimentos.length > 0
    ...
    : 0
  const totalAportado = aportes.filter(a => a.status !== 'Cancelado').reduce((s, a) => s + a.valor, 0)
  ...
  function openEdit(inv: InvestimentoPosition) { const { id, ...rest } = inv; setEditId(id); setForm(rest); setShowForm(true) }
  function handleSave() {
    if (!form.nome) return
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Portfólio e calculadora de projeção</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-700">Posições</h2>
            <button onClick={openAdd} className="text-xs bg-saldo/15 text-saldo px-3 py-1.5 rounded-lg hover:bg-saldo/25 transition-colors font-medium">
              + Adicionar posição
            </button>
          </div>
          {investimentos.length === 0 ? (
            ...
            <div className="divide-y divide-gray-100">
              {investimentos.map(inv => {
                ...
                return (
                  <div key={inv.id} className="px-4 py-3 hover:bg-slate-50 flex items-center justify-between gap-4">
                    <div className="min-w-0">
                      <div className="font-medium text-gray-800 text-sm">{inv.nome}</div>
                      <div className="text-xs text-gray-400 mt-0.5">{inv.tipo} · {inv.instituicao} · {inv.liquidez}</div>
                      {invAportado > 0 && (
                    ...
                    <div className="flex items-center gap-6 shrink-0">
                      <div className="text-right">
                        <div className="text-sm font-medium text-gray-800">{brl(inv.saldoAtual)}</div>
                        <div className="text-xs text-gray-400">{inv.taxaAnual}% a.a. {inv.taxaTipo}</div>
                      </div>
                      <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: ALLOC_COLORS[inv.alocacao] + '20', color: ALLOC_COLORS[inv.alocacao] }}>
                        {inv.alocacao}
                      </span>
                      <div className="text-right text-xs">
                        <div className="text-gray-500">5 anos: <span className="text-receita font-medium">{brl(proj.ano5)}</span></div>
                      </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Alocação</h2>
          {allocData.length === 0 ? (
            <div className="text-center text-sm text-gray-400 py-8">Sem dados</div>
          ) : (
            <>
              ...
              <div className="space-y-2 mt-2">
                {allocData.map(entry => (
                  <div key={entry.name} className="flex justify-between text-xs">
                    ...
                    <div className="text-right">
                      <span className="text-gray-700">{brl(entry.value)}</span>
                      {totalSaldo > 0 && <span className="text-gray-400 ml-1">({pctNum(entry.value / totalSaldo * 100)})</span>}
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        <h2 className="text-sm font-semibold text-gray-700 mb-6">Calculadora de Projeção</h2>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Saldo inicial</label>
            <CurrencyInput
              value={projSaldo}
            ...
            {totalSaldo > 0 && projSaldo === 0 && (
              <div className="text-xs text-gray-400 mt-1">Usando portfólio atual</div>
            )}
          </div>
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Aporte mensal</label>
            <CurrencyInput value={projAporte} onChange={setProjAporte} className="w-full" />
          </div>
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Taxa anual (%)</label>
            <input
              type="number"
              value={projTaxa}
              onChange={e => setProjTaxa(parseFloat(e.target.value) || 0)}
              className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:border-saldo/60"
              step="0.1"
            />
          </div>
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Horizonte: {projHorizonte} anos</label>
            <input
              type="range"
              min="1" max="30"
              value={projHorizonte}
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[{ label: '1 Ano', val: ano1, months: 12 }, { label: '3 Anos', val: ano3, months: 36 }, { label: '5 Anos', val: ano5, months: 60 }].map(ms => {
            ...
            return (
              <div key={ms.label} className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                <div className="text-xs text-gray-500 font-medium mb-2">{ms.label}</div>
                <div className="text-xl font-bold text-gray-800">{brl(ms.val)}</div>
                <div className="text-xs text-gray-400 mt-1">Investido: {brl(invested)}</div>
        ...
        <ResponsiveContainer width="100%" height={240}>
          <AreaChart data={chartData}>
            ...
            <Area type="monotone" dataKey="Saldo" stroke="#0891b2" fill="#0891b220" strokeWidth={2} />
            <Area type="monotone" dataKey="Investido" stroke="#2563eb" fill="none" strokeWidth={1.5} strokeDasharray="5 5" />
          </AreaChart>
        </ResponsiveContainer>
        ...
        {investimentos.length > 0 && (
          <div className="mt-6">
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Projeção por Posição (sem aportes adicionais)</h3>
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {investimentos.map(inv => {
                    ...
                    return (
                      <tr key={inv.id} className="border-b border-gray-100 hover:bg-slate-50">
                        ...
                        <td className="px-3 py-2 text-gray-600">{brl(inv.saldoAtual)}</td>
                        <td className="px-3 py-2 text-gray-500">{inv.taxaAnual}%</td>
                  ...
                  {investimentos.length > 1 && (
                    <tr className="bg-gray-50">
                      ...
                      <td className="px-3 py-2 font-semibold text-gray-800">{brl(totalSaldo)}</td>
                      <td className="px-3 py-2 text-gray-500">{taxaMedia.toFixed(1)}%*</td>
              ...
              {investimentos.length > 1 && <div className="text-xs text-gray-400 mt-2">* Taxa média ponderada pelo saldo</div>}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <div className="px-4 py-3 border-b border-gray-200">
          <h2 className="text-sm font-semibold text-gray-700">Aportes por Posição</h2>
        </div>
                  ...
                  .map(a => {
                    ...
                    return (
                      <tr key={a.id} className="border-b border-gray-100 hover:bg-slate-50">
                        <td className="px-3 py-2.5 text-gray-600">{a.mesAno}</td>
                        <td className="px-3 py-2.5 text-gray-800">{inv?.nome ?? '—'}</td>
                        <td className="px-3 py-2.5 text-saldo font-medium">{brl(a.valor)}</td>
                        <td className="px-3 py-2.5">
                          <span className={`text-xs px-2 py-0.5 rounded-full ${
                            a.status === 'Confirmado' ? 'bg-receita/15 text-receita' : 'bg-alerta/15 text-alerta'
                          }`}>{a.status}</span>
                        </td>
                      </tr>
                    )
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editId ? 'Editar Posição' : 'Nova Posição'} size="lg">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="block text-xs text-gray-600 mb-1 font-medium">Nome do ativo</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Instituição</label>
            <input
              type="text"
              className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:border-saldo/60"
          ...
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Tipo de taxa</label>
          ...
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Valor aplicado</label>
            <CurrencyInput value={form.valorAplicado} onChange={v => setForm(f => ({ ...f, valorAplicado: v }))} className="w-full" />
          </div>
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Saldo atual</label>
            <CurrencyInput value={form.saldoAtual} onChange={v => setForm(f => ({ ...f, saldoAtual: v }))} className="w-full" />
          </div>
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Liquidez</label>
          ...
          <div>
            <label className="block text-xs text-gray-600 mb-1 font-medium">Alocação</label>
